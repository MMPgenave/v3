import { BackgammonBoard, BackgammonStartingRoll, GameInfoButton } from "./board-ui";
import { socketEmit } from "@/app/lib/hooks/hooks";
import { Player, RoomStep, Variant } from "@/app/lib/scripts/backgammon/util";
import { cloneBoard } from "@/app/lib/scripts/backgammon/game";

export function Game({ player, roomStep, startingRolls, variant, boardState, score, roomName }) {
  ...
  return boardState === null ? null : (
    <BackgammonBoard
      ...
      isTurn={!process.env.GAMEDEV ? player === boardState.turn : true}
      doMove={doMove}
      ...
      startingRollW={
        roomStep === RoomStep.startingRoll && (
          <BackgammonStartingRoll startingRolls={startingRolls} player={player} color={Player.white} />
        )
      }
      startingRollB={
        roomStep === RoomStep.startingRoll && (
          <BackgammonStartingRoll startingRolls={startingRolls} player={player} color={Player.black} />
        )
      }
      // gameInfoButton={
      //   <GameInfoButton player={player} score={score} roomName={roomName} />
      // }
    />
  );
}
